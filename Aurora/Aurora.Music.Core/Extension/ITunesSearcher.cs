...
namespace Aurora.Music.Core.Extension
{
    public static class ITunesSearcher
    {
        ...
        {
            string res = await ApiRequestHelper.HttpGet
                (string.Format(lookupUrl, onlineAlbumID));
            
            ITunesSearchResult result = 
                JsonConvert.DeserializeObject<ITunesSearchResult>(res);

            // RnD : TMP
            /*
            return result.results.ConvertAll
            (
                a => new OnlineMusicItem(a.trackName, 
                a.artistName, a.feedUrl, 
                null, a.feedUrl, a.artworkUrl100)
            {
