...
namespace TagLib.Xmp
{
	...
	public class XmpTag : ImageTag
	{
		...
		public XmpTag (string data, File file)
		{
			...
			XmlNode node = doc.CreateCDataSection("/x:xmpmeta/rdf:RDF");//.SelectSingleNode ("/x:xmpmeta/rdf:RDF", nsmgr);
			
			// Old versions of XMP were called XAP, fall back to this case (tested in sample_xap.jpg)
			node = node ?? doc.CreateCDataSection("/x:xmpmeta/rdf:RDF");//.SelectSingleNode ("/x:xapmeta/rdf:RDF", nsmgr);
